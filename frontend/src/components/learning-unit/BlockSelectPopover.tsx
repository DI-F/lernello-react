...
export function BlockSelectPopover({
  index,
  learningUnitId,
}: BlockSelectPopoverProps) {
  ...
  const addBlockMutation = useMutation({
    mutationFn: (blockData: any) =>
      apiClient.getBlockId({ learningUnitId }).then(({ blockId }) => {
        const action = {
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        <PopoverContent className="w-auto p-0">
          <div className="flex">
            <Button
              variant="ghost"
              className="justify-start rounded-none rounded-l-md"
              onClick={createBlockHandler(BlockType.THEORY)}
            >
            ...
            <Button
              variant="ghost"
              className="justify-start rounded-none border-x"
              onClick={createBlockHandler(BlockType.MULTIPLE_CHOICE)}
            >
            ...
            <Button
              variant="ghost"
              className="justify-start rounded-none rounded-r-md"
              onClick={createBlockHandler(BlockType.QUESTION)}
            >
