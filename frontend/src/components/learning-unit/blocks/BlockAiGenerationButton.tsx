...
const mockApiCall = (payload: any) => {
	...
	return new Promise((resolve) => setTimeout(() => resolve({ ...payload, generated: true }), 1000));
};

// Mock Modal components for demonstration
const GenerateQuizModal = ({ isOpen, onConfirm, onCancel, theoryBlocks }: any) =>
	isOpen ? (
		<div>
			<h2>Generate Quiz</h2>
...
export function BlockAiGenerationButton({
	...
	const useGenericMutation = (mutationFn: (payload: any) => Promise<any>, actionType: string) => {
		return useMutation({
			mutationFn,
			onMutate: () => {
				toaster.create({
			...
			onSuccess: (data: any) => {
				queueBlockAction({ type: 'UPDATE_BLOCK', blockId: block.uuid, ...data });
				toaster.create({
			...
			case 'MULTIPLE_CHOICE':
				return (
					<GenerateQuizModal
						...
						theoryBlocks={theoryBlocks.map((b) => ({ id: b.uuid, title: b.name }))}
					/>
				);
			case 'QUESTION':
				return (
