...
export function BlockIconHeader({
  block,
  role,
  language,
  onGenerationLoadingChange,
}: BlockIconHeaderProps) {
  ...
  const initialName = useMemo(() => {
    return (
      block?.translatedContents?.find((c) => c.language === language)?.name ??
      block.name
    );
  }, [block, language]);
    ...
    <div className="flex w-full items-center justify-between">
      ...
      {role === INSTRUCTOR_ROLE && (
        <div className="ml-auto">
          <BlockAiGenerationButton
            block={block}
            onGenerationLoadingChange={onGenerationLoadingChange}
          />
        </div>
      )}
    </div>
