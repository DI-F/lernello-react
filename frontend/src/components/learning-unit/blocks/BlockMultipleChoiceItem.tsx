...
const useTranslation = () => ({
  t: (key: string) => key.split(".").pop()?.replace(/_/g, " ") || "",
});
const useToaster = () => ({
  create: (toast: any) =>
...
const useLearningUnitProgress = () => ({
  getBlockProgress: (
    blockId: string,
  ): MultipleChoiceBlockProgressRes | undefined => {
...
const api = (fetch: any) => ({
  req: (endpoint: any, payload: any) => ({
    parse: () => {
      ...
      const isCorrect = payload.answers.some((a: string) =>
...
export function MultipleChoiceBlockComponent({
  block,
  role,
  language,
}: BlockMultipleChoiceItemProps) {
  ...
  const { initialQuestion, initialAnswers, blockId } = useMemo(() => {
    const translation = block.translatedContents?.find(
      (c) => c.language === language,
    );
    const possibleAnswers =
      translation?.possibleAnswers ?? block.possibleAnswers;
  ...
  useEffect(() => {
    if (role === TRAINEE_ROLE && progress?.lastAnswers) {
      const initialSelected = initialAnswers
        .map((answer, index) =>
          progress.lastAnswers?.includes(answer.value) ? index : -1,
        )
  ...
  const handleSubmit = () => {
    if (selectedAnswers.length > 0) {
      const answersToSubmit = selectedAnswers.map(
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          {!isSubmitted && (
            <Button
              ...
              disabled={
                selectedAnswers.length === 0 || checkAnswerMutation.isPending
              }
            >
