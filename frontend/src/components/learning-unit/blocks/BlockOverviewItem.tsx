  ...
  const { StatusIconComponent, statusColorClass } = useMemo(() => {
    if (role === TRAINEE_ROLE && progress) {
      if (progress.blockType === "MULTIPLE_CHOICE") {
        ...
        if (mcProgress.isCorrect)
          return {
            StatusIconComponent: statusIconMap.CORRECT,
            statusColorClass: "bg-success text-success-foreground",
          };
        if (mcProgress.isCorrect === false)
          return {
            StatusIconComponent: statusIconMap.FALSE,
            statusColorClass: "bg-destructive text-destructive-foreground",
          };
      } else if (progress.blockType === "QUESTION") {
        const qProgress = progress as QuestionBlockProgressRes;
        if (qProgress.isCorrect)
          return {
            StatusIconComponent: statusIconMap.CORRECT,
            statusColorClass: "bg-success text-success-foreground",
          };
        if (qProgress.isCorrect === false)
          return {
            StatusIconComponent: statusIconMap.FALSE,
            statusColorClass: "bg-destructive text-destructive-foreground",
          };
        ...
        if (tProgress.isViewed)
          return {
            StatusIconComponent: statusIconMap.VIEWED,
            statusColorClass: "bg-success text-success-foreground",
          };
      }
    }
    return {
      StatusIconComponent: statusIconMap.ELSE,
