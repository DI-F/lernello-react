...
export function QuestionBlockComponent({
  ...
  const { currentQuestion, currentExpectedAnswer, blockId } = useMemo(() => {
    ...
    return {
      ...
      currentExpectedAnswer:
        translation?.expectedAnswer ?? block.expectedAnswer,
  ...
  const [instructorExpectedAnswer, setInstructorExpectedAnswer] = useState(
    currentExpectedAnswer,
  );
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          ...
          {!isSubmitted && (
            <Button
              ...
              disabled={
                traineeAnswer.trim() === "" || checkAnswerMutation.isPending
              }
        ...
        <div className="flex-grow">
          <label
            htmlFor={`student-answer-${blockId}`}
              ...
              <>
                ...
                {currentExpectedAnswer.trim() !== "" && (
                  <p className="mt-1 text-sm">
                    {t("block.expectedAnswerWas")}:{" "}
                    <code className="font-mono">{currentExpectedAnswer}</code>
                  </p>
                )}
              </>
            )}
